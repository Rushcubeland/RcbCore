...
public enum BattlePassUnit {
    ...
    private String[] getDataOfBattlePassFromMySQL() {
            ...
            try {
                SQL.query(DatabaseManager.Main_BDD.getDatabaseAccess().getConnection(), String.format("SELECT FROM battlepass WHERE name='%s'", getName()), rs -> {
                    try {
                        if(rs.next()) {
    ...
    private void sendDataOfBattlepassToMysql() {
        if(existsTable) {
            try {
                SQL.update(DatabaseManager.Main_BDD.getDatabaseAccess().getConnection(), String.format("UPDATE battlepass SET name='%s', start='%s', end='%s' WHERE name='%s'",
        ...
        {
            try {
                SQL.update(DatabaseManager.Main_BDD.getDatabaseAccess().getConnection(), String.format("INSERT INTO battlepass (name, start, end) VALUES ('%', '%', '%')",
                        getName(), getStart(), getEnd()));
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
